...
class _ProductionFormWidgetState extends State<ProductionFormScreen> {
  ...
  void _checkQuantitywithFinalQuantity() async {
    ...
    if (!errorString.isEmpty) {
      ...
      hdrRequest.insDate = sendingDateFormat.format(now);
      hdrRequest.docDate = sendingDateFormat.format(now);
      hdrRequest.udtDate = sendingDateFormat.format(now);
      hdrRequest.docFinyear = int.parse(
          "${int.parse(yearDate.format(now)) - 1}${int.parse(yearDate.format(now))}");
  ...
  Future<List<Map>> getPendingOrderRequests(
    ...
    for (var selected in allSelected) {
      ...
      pendingOrderRequest.insDate = sendingDateFormat.format(now);
      pendingOrderRequest.udtDate = sendingDateFormat.format(now);
  ...
  void getAllStock(int pk_code, int? prodOrderDtlPk, [int? qtyuom]) async {
    ...
    for (var good in goods) {
      ...
      stockRequest.insdate = sendingDateFormat.format(now);
      ...
      stockRequest.udtdate = sendingDateFormat.format(now);
