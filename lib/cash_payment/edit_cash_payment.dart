...
class EditCashPayment extends StatefulWidget {
  final String? compId;
  ...
  @override
  State<StatefulWidget> createState() => _EditCashPayment();
}

class _EditCashPayment extends State<EditCashPayment> with NetworkResponse {
  late int index;
...
// bool readOnly=true;
  ...
  void initState() {
    super.initState();
    logIt('acccode->${widget.partyCode}');
    _init();
  }
  ...
  _getSave() {
    try {
      ...
      Map jsonBody = {
        ...
        'pur_bill_hdr_pk':
            _paytypeController.text == 'AGAINST BILL' ? salePurHdrPk : '',
        'item_pay_amount':
      ...
      WebService.fromApi(AppConfig.cashSave, this, jsonBody)
          .callPostService(context);
    } catch (err, stack) {
  ...
  _getPaymentDetail() {
    ...
    WebService.fromApi(AppConfig.cashPaymentDetail, this, jsonBody)
        .callPostService(context);
  }
  ...
  void onResponse({Key? key, String? requestCode, String? response}) {
    switch (requestCode) {
      ...
      case AppConfig.cashSave:
        ...
        break;
      case AppConfig.cashPaymentDetail:
        {
          ...
          if (resp['error'] == 'false') {
            ...
            var pendingbillcontent = data['payment_items'] as List?;
            if (pendingbillcontent != null) {
              pendingBillList.addAll(pendingbillcontent
                  .map((e) => PendingBillModel.parseItems(e))
                  .toList());
            }
            if (pendingBillList.isNotEmpty) {
              ///Header
      ...
      case AppConfig.getDelete:
        {
          ...
          if (data['error'] == 'false') {
            showAlert(context, data['message'], 'Success', onOk: () {
              pendingBillList.removeAt(index);
              if (pendingBillList.length <= 2) {
                pendingBillList.clear();
                _diffAmountController.clear();
              }
              setState(() {});
                              ...
                              children: [
                                ...
                                Visibility(
                                  ...
                                  child: IconButton(
                                      ...
                                      onPressed: () {
                                        if (pendingBillList[index].isLocal) {
                                          pendingBillList.removeAt(index);
