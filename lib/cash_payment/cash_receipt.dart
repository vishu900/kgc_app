...
class CashReceipt extends StatefulWidget {
  ...
  const CashReceipt(
      {Key? key, required this.compId, this.partyCode, this.compName})
      : super(key: key);

  @override
  State<StatefulWidget> createState() => _CashReceipt();
}

class _CashReceipt extends State<CashReceipt> with NetworkResponse {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Scaffold(
        appBar: AppBar(title: Text('Cash Receipt')),
  ...
  _getReceiptSubmit() {
    ...
    receiptBillList.forEach((element) {
      if (receiptBillList.first != element && receiptBillList.last != element) {
        ...
        if (itemReceiptAmount.isEmpty) {
          itemReceiptAmount = element.amount;
        } else {
          itemReceiptAmount = '$itemReceiptAmount,${element.amount}';
        }
      }
    });
    ...
    Map jsonBody = {
      ...
      'item_receipt_amount': _paytypeController.text == 'AGAINST BILL'
          ? itemReceiptAmount
    ...
    logIt('_getReceiptSubmit -> $jsonBody ');

    WebService.fromApi(AppConfig.cashReceiptSubmit, this, jsonBody)
        .callPostService(context);
  }
  ...
  void onResponse({Key? key, String? requestCode, String? response}) {
    switch (requestCode) {
      case AppConfig.receiptantPartyDetail:
        ...
        break;
      case AppConfig.cashReceiptSubmit:
        {
          var data = jsonDecode(response!);
          if (data['error'] == 'false') {
