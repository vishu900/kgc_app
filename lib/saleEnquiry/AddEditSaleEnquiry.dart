...
import 'package:dataproject2/quotation/AddressModel.dart';
import 'package:dataproject2/quotation/PartyModel.dart';
import 'package:dataproject2/quotation/SendEmail.dart';
...
class AddEditSaleEnquiry extends StatefulWidget {
  final qtnId;

  const AddEditSaleEnquiry({Key? key, this.qtnId}) : super(key: key);

  @override
  State<StatefulWidget> createState() => _AddEditSaleEnquiry();
}

class _AddEditSaleEnquiry extends State<AddEditSaleEnquiry>
  ...
  TextEditingController basicRateController = TextEditingController();
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          widget.qtnId == null
              ? 'Sale Enquiry'
              : (isEditable ? 'Edit Sale Enquiry' : 'View Sale Enquiry'),
          style: TextStyle(
            letterSpacing: 2.17,
          ...
          Visibility(
              ...
              child: Visibility(
                visible: ifHasPermission(
                    ...
                    permission: Permissions.SALE_ENQ)!,
                child: IconButton(
                                            ...
                                            builder: (BuildContext context,
                                              ...
                                              return PhotoViewGalleryPageOptions(
                                                imageProvider: NetworkImage(
                                                    '$imageBaseUrl${itemList[index].imageList![imgIndex]}.png'),
                                                initialScale:
 ...
 */
  ...
  _downloadPdf(String? docCode) async {
    ...
    var url = '${AppConfig.downloadSaleEnq}$docCode';
  ...
  _saveQtnForm() async {
    ...
    //  return;
    WebService()
        .post(this.context, AppConfig.addEnquiry, json)
        .then((value) => {
              Navigator.of(this.context).pop(),
              if (value!.statusCode == 200)
                {
  ...
  _updateQtnForm() async {
    ...
    WebService()
        .post(this.context, AppConfig.editEnquiry, json)
        .then((value) => {
              if (value!.statusCode == 200)
                {
                  if (jsonDecode(value.body)['error'] == 'true')
  ...
  _getQtnDetail() async {
    ...
    WebService().post(this.context, AppConfig.getEnquiryDetail, json).then(
  ...
  _parseQtnDetail(Response value) {
    if (value.statusCode == 200) {
      ...
      _entryDateController.text = Commons().getFormattedDate(
